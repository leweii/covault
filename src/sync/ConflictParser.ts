...
// so each hunk can be resolved independently (Take Local / Remote / Edit /
// Skip). Ported from agentic-git-sync.
  ...
  while (i < lines.length) {
    ...
    if (line.startsWith(MARK_START)) {
      ...
      while (i < lines.length && !(lines[i] ?? "").startsWith(MARK_END)) {
        const l = lines[i] ?? "";
        if (l.startsWith(MARK_BASE)) {
          phase = "base";
        } else if (l.startsWith(MARK_SEP)) {
          phase = "remote";
        } else {
          if (phase === "local") local.push(l);
          else if (phase === "remote") remote.push(l);
...
export function applyResolutions(segments: ConflictSegment[], resolutions: Map<string, HunkResolution>): string {
  const out: string[] = [];
  for (const seg of segments) {
    if (seg.kind === "common") {
      out.push(...seg.lines);
...
export function isFullyResolved(segments: ConflictSegment[], resolutions: Map<string, HunkResolution>): boolean {
  for (const seg of segments) {
...
export function getContextLines(
  ...
  const idx = segments.findIndex((s) => s.kind === "conflict" && s.hunk.id === hunkId);
  ...
  for (let i = idx - 1; i >= 0 && before.length < n; i--) {
    ...
    if (seg?.kind === "common") {
      ...
      before.unshift(...seg.lines.slice(-remaining));
    }
  }
